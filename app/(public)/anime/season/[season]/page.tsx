...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const persianSeason = seasonMap[season.toLowerCase()] || season;

  return {
    title: `دانلود و تماشای انیمه‌های فصل ${persianSeason} | AoYume`,
    alternates: {
      canonical: `/anime/season/${season}`,
    },
  };
}
...
export default async function SeasonPage({ params, searchParams }: Props) {
  ...
  if (!validSeasons.includes(seasonLower)) {
    return (
      <div className="container mx-auto min-h-screen px-4 py-8 text-center">
        <p className="text-muted-foreground text-lg">فصل نامعتبر است.</p>
      </div>
    );
  }
  ...
  let queryBuilder = supabase
    ...
    .select("*", { count: "exact" })
    .eq("post_status", 1)
    .eq("season", seasonLower as Database["public"]["Enums"]["season"]);
  ...
  const persianSeason = seasonMap[seasonLower] || season;
    ...
    <div className="container mx-auto min-h-screen px-4 py-8">
      <div className="mb-8 flex flex-col items-center justify-center space-y-2 md:mb-12">
        <h1 className="text-primary text-3xl font-extrabold tracking-tight md:text-4xl">
          انیمه‌های فصل {persianSeason}
        </h1>
        <p className="text-muted-foreground text-lg">
          لیست کامل انیمه‌های پخش شده در فصل {persianSeason} با زیرنویس فارسی
        </p>
      </div>
      ...
      {items.length > 0 ? (
        ...
        <div className="text-muted-foreground py-12 text-center">
          هیچ انیمه‌ای در این فصل یافت نشد.
        </div>
      )}
    </div>
  );
