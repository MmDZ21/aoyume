...
const EpisodeContainer = ({ item }: { item: DownloadItem }) => {
  ...
  return (
    <div className="group relative flex aspect-video w-full flex-col justify-end overflow-hidden rounded-2xl bg-muted text-foreground shadow-md transition-all duration-300 hover:shadow-xl md:aspect-video">
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-background/60 opacity-100 backdrop-blur-[2px] transition-all duration-300 sm:opacity-0 sm:group-hover:opacity-100" />
      </div>

      {/* Top Badges */}
      <div className="absolute left-4 top-4 z-20 flex gap-2">
         <span className="rounded-full bg-background/80 px-3 py-1 text-xs font-medium text-foreground backdrop-blur-md transition-all duration-300 group-hover:bg-primary/80 group-hover:text-primary-foreground">
          {item.resolution}
        </span>
      </div>
      ...
      <div className="relative z-10 w-full p-4 sm:p-6">
        <div className="transform transition-all duration-300 sm:translate-y-8 sm:group-hover:translate-y-0">
          ...
          <div className="flex items-end justify-between">
            <div>
              <h3 className="text-xl font-bold leading-none tracking-tight text-foreground drop-shadow-md sm:text-2xl">
                قسمت {item.episode}
              </h3>
            </div>
          </div>
